...
namespace VOffice.API.Controllers.Api.Account
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> ResetPassword(ResetPasswordBindingModel model)
        {
            ...
            var user = UserManager.GeneratePasswordResetToken(model.UserId);

            IdentityResult result = UserManager.ResetPassword(model.UserId, user, model.NewPassword);
            if (!result.Succeeded)
            {
                return GetErrorResult(result);
            }
        ...
        [AllowAnonymous]
        public async Task<IHttpActionResult> RestorePassword(ResetPasswordViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var user = new ApplicationUser() { UserName = model.Email, Email = model.Email, DepartmentId = model.DepartmentId, GoogleAccount = model.GoogleAccount, ExpiryDate = model.ExpiryDate, Deleted = false, CreatedOn = DateTime.Now, CreatedBy = model.CreatedBy, EditedOn = DateTime.Now, EditedBy = model.EditedBy };
